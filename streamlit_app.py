...
# --- FUNZIONE CHIAMATA AI (GEMINI PRO - VERSIONE STABILE) ---
def get_gemini_response(prompt):
    try:
        # Usiamo 'gemini-pro' classico che è il più compatibile
...
def get_gemini_search(query, ctx):
    try:
        model = genai.GenerativeModel('gemini-pro')
        response = model.generate_content(f"{ctx} Query: {query}")
        return response.text
    except Exception as e:
        return f"Ricerca non disponibile: {str(e)}"
