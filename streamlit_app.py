...
def process_image(image_file, border_width):
    ...
    # Resize smart: manteniamo le proporzioni ma non tagliamo a cerchio
    if border_width > 0:
...
# --- 6. CREATE CV DOCX (FIX LAYOUT) ---
def create_cv_docx(json_data, pil_image, lang_code):
    doc = Document()
    
    # 1. HEADER TABLE (FOTO SX - TESTO DX)
    table = doc.add_table(rows=1, cols=2)
    table.autofit = False
    
    # Larghezze colonne: Sx stretta per foto, Dx larga per testo
    ...
    if pil_image:
        ...
        p_foto.alignment = WD_ALIGN_PARAGRAPH.LEFT # Allineato a sinistra
        # Rimuovi margini per centratura perfetta
        p_foto.paragraph_format.space_before = Pt(0)
        p_foto.paragraph_format.space_after = Pt(0)
        p_foto.paragraph_format.line_spacing = 1.0
        
        run_foto = p_foto.add_run()
        run_foto.add_picture(img_s, height=Inches(1.5)) # Altezza fissa
    ...
    p_name.alignment = WD_ALIGN_PARAGRAPH.LEFT # Vicino alla foto
    ...
    info_str = f"{json_data['personal_info'].get('address','')} | {json_data['personal_info'].get('phone','')} | {json_data['personal_info'].get('email','')}"
    run_info = p_info.add_run(info_str)
        ...
        if key in json_data['cv_sections'] and json_data['cv_sections'][key]:
            ...
            if isinstance(items, list):
                for item in items:
                    ...
                    # SPAZIO TRA VOCI SOLO PER EXP/EDU
